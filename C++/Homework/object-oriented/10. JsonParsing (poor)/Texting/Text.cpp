...
{
	StringX* newText = new StringX[nLength];
...
Text::Text(const char* sDiscription)
{
	ptrText = nullptr;
...
Text::Text(Text& anotherText, StringX discription)
{
	int lines = anotherText.getLinesQuantity();
...
Text::Text(char** someText, int size, StringX discription)
{
	ptrText = new char* [size];

	for (size_t i = 0ULL; i < size; i++)
	{
		ptrText[i] = new char[strlen(someText[i]) + TERMINAL];
	}
...
void Text::setText(char** newText, int size)
{
	...
	for (size_t i = 0ULL; i < size; i++)
	{
		ptrText[i] = new char[strlen(newText[i]) + TERMINAL];
	}
...
void Text::setText(StringX* newText, int size)
{
	delete[]ptrText;
	ptrText = new char* [size];
...
StringX Text::getDiscription()
{
	return sDiscription;
}


void Text::setDiscription(StringX sDiscription)
{
	this->sDiscription = sDiscription;
}

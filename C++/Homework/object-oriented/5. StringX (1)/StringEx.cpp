...
int StringEx::contains(StringEx sSearchedString)
{
	int nReturnValue = NEGATIVE;
	...
	const char* sSearchedStringPtr = sSearchedString.getArrayPtr();

	for (int i = 0; sSearchedStringPtr[i] != '\0'; i++)
	{
		nSearchedSize += 1;
	}
	...
	for (int i = 0; i < nLength; i++)
	{
		...
		if (ptrArray[i] == sSearchedStringPtr[0])
		{
			nReturnValue = i;

			for (int j = 0, k = i; j < nSearchedSize && k < nLength; j++, k++)
			{
				if (ptrArray[k] == sSearchedStringPtr[j]) counter++;
			}
